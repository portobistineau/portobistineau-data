// weather.js - FULLY FIXED & OPTIMIZED (Dec 2025)
// For portobistineau.com - Lake Bistineau Weather + Radar Widget
// Fixes infinite "Fetching..." by adding proper caching + timeouts
...
async function updateWeather() {
    ...
    const CACHE_KEY = 'nwsCache_v4';           // bumped version to clear old bad cache
    ...
    try {
        ...
        document.getElementById('bistineauWeather').innerHTML =
            '<div style="padding:20px;color:#900;background:#fff0f0;border-radius:8px;text-align:center;">Partial data – refreshing soon</div>';
        return;
    }
...
// ==================================================================
// RADAR WIDGET (unchanged except minor safety improvements)
// ==================================================================
...
function initRadarWidget() {
    ...
    fetch(dotdGeoJsonUrl)
        ...
        .then(data => {
            if (data.features && data.features.length > 0) {
                L.geoJson(data, { style: lakeStyle }).addTo(map);
            }
        })
        .catch(() => {});
    ...
    // Marina marker + label
    var marinaLat = 32.4619, marinaLng = -93.34883;
    ...
    L.marker([marinaLat + 0.01, marinaLng - 0.01], { icon: textLabelIcon }).addTo(map);
    ...
    fetch('https://api.rainviewer.com/public/weather-maps.json')
        .then(r => r.json())
        .then(data => processRadarFrames(map, data))
...
function processRadarFrames(map, apiData) {
    ...
    var frames = past.slice(-24).concat(future.slice(0, 12));

    frames.forEach((frameObj, index) => {
        var layer = L.tileLayer(apiData.host + frameObj.path + '/256/{z}/{x}/{y}/2/1_1.png', {
            opacity: 0,
            zIndex: 100
        });
        layer.isFuture = (index >= past.length);
...
function updateRadarDisplay() {
    ...
    if (document.getElementById('radar-time') && document.getElementById('radar-status')) {
        document.getElementById('radar-time').textContent = formatTime(currentLayer.timestamp);
        document.getElementById('radar-status').textContent = currentLayer.isFuture ? "FUTURE" : "PAST";
        document.getElementById('radar-status').style.color = currentLayer.isFuture ? "#00ccff" : "#00ff00";
    }
}
